...
from src.services.ai_config import load_ai_config, get_model, get_system_prompts, get_templates
...
IMAGE_MODEL = get_model("image_generation", config=_AI_CONFIG)  # Model that supports image generation
...
class ImageService:
    ...
    def _validate_prompt(self, prompt: str) -> str:
        ...
        if len(cleaned) > MAX_PROMPT_CHARS:
            raise ImageGenerationError(f'Prompt is too long (max {MAX_PROMPT_CHARS} characters)')
        return cleaned
    ...
    def _validate_attachments(
        ...
        if reference_images:
            ...
            existing = [p for p in reference_images if isinstance(p, Path) and p.exists() and p.is_file()]
    ...
    async def _generate_image_impl(
        ...
        logger.info(f"P-THRESHOLD: {p_threshold}, TEMPERATURE: {temperature}")
        logger.info("-"*60)
        logger.info("PROMPT:")
        ...
        api_result = await loop.run_in_executor(
            None,
            lambda: self._call_api(contents, config)
        )
    ...
    def _call_api(
        ...
        try:
            ...
            for chunk in self._client.models.generate_content_stream(
                ...
                if image_bytes is not None and mime_type is not None and usage is not None and usage.total_tokens is not None:
                    break
    ...
    def get_thumbnail_path(self, image_path: Path) -> Optional[Path]:
        ...
        """
        thumbnail_path = self._working_folder / ".thumbnails" / f"{image_path.stem}_thumb.png"
