...
class ProjectManager:
    ...
    def remove_image(self, image_id: str) -> bool:
        ...
        try:
            ...
            if file_path.exists():
                ...
                thumb_path = (
                    self._working_folder / ".thumbnails" / f"{file_path.stem}_thumb.png"
                )
    ...
    def move_image(self, image_id: str, new_category: str) -> bool:
        ...
        try:
            ...
            src_thumb = (
                self._working_folder / ".thumbnails" / f"{source_path.stem}_thumb.png"
            )
            if src_thumb.exists():
                dst_thumb = (
                    self._working_folder
                    / ".thumbnails"
                    / f"{target_path.stem}_thumb.png"
                )
    ...
    def _rename_file_and_thumb(self, src: Path, dst: Path):
        try:
            ...
            if src_thumb.exists():
                dst_thumb = (
                    self._working_folder / ".thumbnails" / f"{dst.stem}_thumb.png"
                )
...
class ImageManager:
    ...
    def _build_ui(self) -> None:
        ...
        # Category tabs
        with (
            ui.tabs(value=self._current_tab)
            .classes("w-full")
            .bind_value(self, "_current_tab") as tabs
        ):
            ui.tab("Pages")
            ui.tab("References")
            ui.tab("Inputs")
    ...
    def _build_pages_grid(self) -> None:
        ...
        if not pages:
            ui.label("No pages yet. Generate some pages first!").classes(
                "text-gray-500"
            )
            return
            ...
            def save():
                ...
                if new_name and new_name != current_name:
                    if self._project.rename_image(image_id, new_name):
                        ...
                        ui.notify(
                            "Failed to rename (name might exist)", type="negative"
                        )
                else:
                    dialog.close()
    ...
    def _build_image_card(
        self, image_data: dict, category: str, index: int = -1, total: int = 0
    ) -> None:
        ...
        thumb_path = (
            self._working_folder / ".thumbnails" / f"{Path(image_path).stem}_thumb.png"
        )
        ...
        with ui.card().classes("cursor-pointer hover:shadow-lg transition-shadow"):
            ...
            with ui.element("div").classes(
                ...
                if thumb_path.exists():
                    ui.image(str(thumb_path)).props("fit=contain").classes(
                        "w-full h-full"
                    )
                else:
                ...
                with ui.row().classes("w-full items-center justify-center gap-2 px-2"):
                    if total > 1:
                        if index > 0:
                            ui.button(
                                icon="arrow_back",
                                on_click=lambda: self._move_page(index, -1),
                            ).props("flat dense round size=sm")
                        else:
                            ui.button(icon="arrow_back").props(
                                "flat dense round size=sm disabled"
                            )
                    ...
                    if total > 1:
                        if index < total - 1:
                            ...
                            ui.button(icon="arrow_forward").props(
                                "flat dense round size=sm disabled"
                            )
                        ...
                        try:
                            ...
                            if getattr(e, "winerror", 0) == 1155:
                                try:
                                    ...
                                    ui.notify(
                                        f"Could not open file: {e2}", type="negative"
                                    )
                            else:
                                logger.error(
                                    f"Failed to open {full_path} for editing: {e}"
                                )
                                ui.notify(
        ...
        with ui.dialog() as self._preview_dialog:
            ...
            with ui.element("div").style(
                ...
                with ui.element("div").style(
                    ...
                    ui.button(icon="close", on_click=self._preview_dialog.close).props(
                ...
                with ui.element("div").style(
                    ...
                    self._preview_image = (
                        ui.image("").props("fit=contain").classes("w-full h-full")
                    )
                    ...
                    with ui.element("div").classes(
                        ...
                        ui.button(
                            icon="chevron_left", on_click=self._prev_preview_image
                        ).props("flat round color=white size=lg").classes(
                            "pointer-events-auto bg-black/30 hover:bg-black/50"
                        )
                        ui.button(
                            icon="chevron_right", on_click=self._next_preview_image
                        ).props("flat round color=white size=lg").classes(
                            "pointer-events-auto bg-black/30 hover:bg-black/50"
                        )
    ...
    def _update_preview_content(self) -> None:
        ...
        if (
            not self._preview_images
            or self._preview_index < 0
            or self._preview_index >= len(self._preview_images)
        ):
            return
